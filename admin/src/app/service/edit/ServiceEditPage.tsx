...
export default function ServiceEditPage() {
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                ...
                // Set image and determine if it's a URL or uploaded file
                if (service.image) {
                    setImage(service.image);
                    setImagePreview(service.image);
                    setImageUrl(service.image);
                    // If image contains 'localhost' or '/uploads', it was uploaded
                    // Otherwise assume it's an external URL
                    if (service.image.includes('/uploads') || service.image.includes('localhost:4000')) {
    ...
    const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            setImagePreview(image); // Revert to previous image
        } finally {
            setUploading(false);
        }
    };
                                ...
                                {!image && !imagePreview ? (
                                    ...
                                    <div className="relative inline-flex items-center gap-4 rounded-xl border border-slate-200 bg-slate-50 p-4">
                                        ...
                                        <div className="flex flex-col">
                                            ...
                                            {/* Replace button */}
                                            <label className="mt-2 cursor-pointer text-xs font-medium text-blue-600 hover:text-blue-700">
                                                Replace image
                                                <input
                                                    type="file"
                                                    accept="image/*"
