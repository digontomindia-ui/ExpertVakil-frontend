// src/pages/subadmins/SubAdminsCenter.tsx
"use client";
...
// ---- Main Component ----
export default function SubAdminsCenter() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        setError(e instanceof Error ? e.message : "Failed to load sub-admins");
      } finally {
  ...
  const roles = useMemo(() => {
    ...
    items.forEach((subadmin) => subadmin.role && s.add(subadmin.role));
  ...
  const visible = useMemo(() => {
    ...
    // role filter
    if (role !== "all") {
      out = out.filter((subadmin) => subadmin.role === role);
    }

    // active filter
    if (active !== "all") {
      const isActive = active === "true";
      out = out.filter((subadmin) => subadmin.isActive === isActive);
    }
      ...
      <main className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
        {loading ? (
          ...
          <div className="rounded-xl border border-red-200 bg-red-50 p-4 text-red-700">
            Failed to load sub-admins: {error}
          </div>
        ) : visible.length === 0 ? (
          <EmptyState
            onReset={() => {
...
function SubAdminCard({ subadmin, onDeleted }: { subadmin: SubAdmin; onDeleted: (id: string) => void }) {
  const [deleting, setDeleting] = React.useState(false);

  const created = tsToDate(subadmin.createdAt);
  ...
  async function handleDelete(e: React.MouseEvent) {
    ...
    const ok = window.confirm(`Delete sub-admin "${subadmin.name}"?`);
    if (!ok) return;

    try {
      setDeleting(true);
      await SubAdminsAPI.remove(subadmin.id);
      onDeleted(subadmin.id);
    } catch (err: any) {
  ...
  return (
    <Link to={`/subadmins/${subadmin.id}`} className="block">
          ...
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-3">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="truncate text-sm text-slate-600">{subadmin.email}</p>
              </div>
            </div>
          ...
          <div className="space-y-2 text-sm text-slate-600">
            {subadmin.phoneNumber && (
              <div className="flex items-center gap-2">
                ...
                <span>{subadmin.phoneNumber}</span>
              </div>
            )}
            {subadmin.allowedTabs && subadmin.allowedTabs.length > 0 && (
              <div className="flex items-center gap-2">
                <span className="text-xs">🔑</span>
                <span>{subadmin.allowedTabs.length} tab{subadmin.allowedTabs.length !== 1 ? 's' : ''} allowed</span>
              </div>
            )}
          </div>
        </div>
...
function EmptyState({ onReset }: { onReset: () => void }) {
  return (
    <div className="mx-auto max-w-md rounded-2xl border border-slate-200 bg-white p-8 text-center shadow-sm">
      ...
      <h3 className="text-lg font-semibold text-slate-900">No sub-admins found</h3>
      <p className="mt-1 text-sm text-slate-600">
        Try clearing filters or create a new sub-admin.
      </p>
      <button
        onClick={onReset}
