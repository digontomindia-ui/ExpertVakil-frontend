...
export default function SubAdminEditPage() {
  ...
  const [form, setForm] = React.useState<Partial<SubAdmin>>({
    name: "",
    email: "",
    phoneNumber: "",
    role: "",
  ...
  React.useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const subadmin: SubAdmin = response.data;
        if (mounted && subadmin) setForm(subadmin);
      } catch (e: any) {
  ...
  function onChange<K extends keyof SubAdmin>(key: K, value: SubAdmin[K]) {
    setForm((prev) => ({ ...prev, [key]: value }));
  }

  function onTabsChange(tabs: string[]) {
    setForm((prev) => ({ ...prev, allowedTabs: tabs }));
  }
  ...
  if (loading) return <PageShell title="Edit Sub-Admin"><Skeleton /></PageShell>;

  return (
    <PageShell title="Edit Sub-Admin">
      <BackBar to={`/subadmins/${id}`} label="← Back to sub-admin" />
      {error && (
      ...
      <form onSubmit={onSubmit} className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        <div className="lg:col-span-2 space-y-6">
          <Card className="p-4 space-y-4">
            <h3 className="text-sm font-semibold text-slate-900">Sub-Admin Information</h3>
            <Input
              label="Full Name"
              value={form.name || ""}
            ...
            <Input
              ...
              placeholder="e.g. Content Manager, Support Admin"
            />
          </Card>
          ...
          <Card className="p-4 space-y-4">
            ...
            <div>
              ...
              <TabsSelector
                selected={form.allowedTabs || []}
                onChange={onTabsChange}
              />
              <p className="text-xs text-slate-500 mt-1">
                Select which admin tabs this sub-admin can access.
              </p>
            </div>
          </Card>
        </div>
        ...
        <div className="space-y-6">
          <Card className="p-4 space-y-4">
            ...
            <div className="text-xs text-slate-600 space-y-1">
              <p>• <strong>Active:</strong> Sub-admin can access the system</p>
              <p>• <strong>Inactive:</strong> Sub-admin access is disabled</p>
            </div>
          </Card>

          <Card className="p-4 space-y-4">
            <h3 className="text-sm font-semibold text-slate-900">Sub-Admin Details</h3>
            <Readonly label="Sub-Admin ID" value={form.id || "—"} />
            <Readonly
              label="Created At"
...
function BackBar({ to = "/subadmins", label = "← Back to list" }: { to?: string; label?: string }) {
  return (
    <div className="mb-4">
...
function TabsSelector({ selected, onChange }: { selected: string[]; onChange: (tabs: string[]) => void }) {
  /**
   * IMPORTANT: These tab keys MUST match the sidebar exactly (PascalCase)
  ...
  function toggleTab(tabKey: string) {
    if (selected.includes(tabKey)) {
      onChange(selected.filter((t) => t !== tabKey));
    } else {
      onChange([...selected, tabKey]);
    }
  }
  ...
  return (
    <div className="space-y-3">
      <div className="rounded-lg bg-blue-50 border border-blue-200 p-3 text-xs text-blue-800">
        <strong>💡 Note:</strong> Dashboard is always accessible to all sub-admins and doesn't need to be selected.
      </div>
      <div className="grid grid-cols-2 gap-2">
        {availableTabs.map((tab) => (
          <label key={tab.key} className="flex items-center space-x-2 p-2 rounded-lg hover:bg-slate-50 cursor-pointer transition">
            <input
              type="checkbox"
      ...
      {selected.length > 0 && (
        <div className="rounded-lg bg-green-50 border border-green-200 p-3">
          <div className="text-xs font-medium text-green-800 mb-1">Selected Tabs ({selected.length}):</div>
          <div className="flex flex-wrap gap-1">
            {selected.map((tabKey) => (
              <span key={tabKey} className="px-2 py-1 bg-green-100 text-green-800 rounded-md text-xs font-medium border border-green-200">
                {tabKey}
              </span>
            ))}
          </div>
